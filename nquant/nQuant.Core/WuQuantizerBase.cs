...
namespace nQuant
{
    public abstract class WuQuantizerBase
    {
        ...
        public Image QuantizeImage(Bitmap image, int maxColors)
        {
            return QuantizeImage(image, maxColors, 10, 70);
        }

        public Image QuantizeImage(Bitmap image, int maxColors, int alphaThreshold, int alphaFader)
        {
            MaxColor = maxColors;
        ...
        {
            var result = new Bitmap(sourceImage.Width, sourceImage.Height, MaxColor > 16 ? PixelFormat.Format8bppIndexed : PixelFormat.Format8bppIndexed);
